    ...
    <div className="admin-page slide-up">
      ...
      {stats && (
        <div className="stat-grid">
          <div className="stat-card"><div className="stat-num">{stats.teams}</div><div className="stat-label">Teams</div></div>
          <div className="stat-card"><div className="stat-num">{stats.totalVotes}</div><div className="stat-label">Total Votes</div></div>
          <div className="stat-card"><div className="stat-num">{stats.uniqueVoters}</div><div className="stat-label">Unique Voters</div></div>
        </div>
      )}
      ...
      <section className="admin-section">
        ...
        <div className="row-gap">
          <input className="input" value={editTitle} onChange={e => setEditTitle(e.target.value)} placeholder="Event title" />
        ...
        <div style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <button className={`btn ${event?.isVotingOpen ? 'btn-danger' : 'btn-success'}`}
            style={{ padding: '6px 14px', fontSize: 13 }} onClick={handleToggleVoting}>
      ...
      <section className="admin-section">
        ...
        <div className="teams-admin-list">
          {teams.map((t, i) => (
            <div key={t._id} className="team-admin-row">
              <span className={`tag team-color-${t.colorIndex % 6}`}
                style={{ width: 28, height: 28, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, border: '1px solid', flexShrink: 0 }}>
                {i + 1}
              </span>
              {editingTeam?._id === t._id ? (
                <>
                  <input className="input" value={editingTeam.name}
                    onChange={e => setEditingTeam(prev => ({ ...prev, name: e.target.value }))}
                    onKeyDown={e => e.key === 'Enter' && handleRenameTeam(t._id)} autoFocus style={{ flex: 1 }} />
        ...
        <div className="row-gap" style={{ marginTop: 12 }}>
          <input className="input" value={newTeamName} onChange={e => setNewTeamName(e.target.value)}
            placeholder="New team name" onKeyDown={e => e.key === 'Enter' && handleAddTeam()} />
          <button className="btn btn-primary" onClick={handleAddTeam} disabled={!newTeamName.trim()}>+ Add Team</button>
        </div>
      </section>
