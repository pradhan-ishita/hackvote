...
export default function VotePage() {
  ...
  if (!event?.isVotingOpen) return (
    <div className="vote-closed card fade-in">
  ...
  if (voted) return (
    <div className="voted-confirm card">
      ...
      <button className="btn" onClick={() => { setVoted(false) }} style={{ marginTop: 16 }}>
        ✏️ Change my vote
      </button>
    </div>
  )
  ...
  return (
    <div className="vote-page slide-up">
      <div className="vote-header">
        <h1>{event?.title}</h1>
        <p className="vote-sub">Select your favourite project and cast your vote.</p>
      </div>

      <p className="teams-count">{teams.length} projects competing</p>
      ...
      <div className="teams-list">
        {teams.map((t, i) => {
          ...
          return (
            ...
            >
              <span className={`team-num tag team-color-${t.colorIndex % 6}`}>
                {i + 1}
              </span>
              <span className="team-card-name">{t.name}</span>
              {isMine && <span className="current-tag">Your vote</span>}
      ...
      {selected && myVoteId && selected !== myVoteId && (
        <div className="change-warning fade-in">
          ⚠️ This will cancel your vote for <strong>{teams.find(t => t._id === myVoteId)?.name}</strong>
        </div>
      )}
      ...
      >
        {submitting
          ...
          : selected ? '🗳 Cast Vote' : 'Select a team to vote'}
      </button>
    </div>
  )
}
